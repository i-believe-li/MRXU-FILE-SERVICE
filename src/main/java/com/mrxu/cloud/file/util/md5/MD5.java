package com.mrxu.cloud.file.util.md5;
...
public class MD5 {
    ...
    /**
     * Calculates and returns the hash of the contents of the given MultipartFile.
     **/
    public static byte[] getHash (MultipartFile file) throws IOException {
        InputStream ins = null;
        InputStream close_me = null;
        try {
            long buf_size = file.getSize();
            byte[] buf = new byte[(int) buf_size];
            ins = file.getInputStream();
            MD5InputStream in = new MD5InputStream(ins);
            close_me = in;
            while (in.read(buf) != -1);
            in.close();
            return in.hash();
